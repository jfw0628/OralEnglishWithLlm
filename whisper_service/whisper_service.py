...
from whisper_service.vad import VoiceActivityDetection
from whisper_service.transcriber import WhisperModel
                ...
                try:
                    ...
                    if speech_prob < self.vad_threshold:
                        ...
                        if no_voice_activity_chunks > 5:
                            ...
                            time.sleep(0.3)    # EOS stop receiving frames for a 100ms(to send output to LLM.)
                        continue
                    no_voice_activity_chunks = 0
...
class ServeClient:
    ...
    def speech_to_text(self):
        ...
        while True:
            ...
            if self.frames_np is None:
                time.sleep(0.05)    # wait for any audio to arrive
                continue
            ...
            samples_take = int(self.timestamp_offset * self.RATE)
            ...
            try:
                ...
                results, info = self.transcriber.transcribe(
                    input_sample,
                    initial_prompt=None,
                ...
                last_segment = ''.join([i.text for i in results])

                if len(last_segment):
                    segments = [{"text": last_segment}]
                    try:
                        self.prompt = last_segment
